...
polynomial* rlibm_solve_with_soplex(psample_data* sintervals,
  ...
  if(stat==SPxSolver::OPTIMAL){
    ...
    polynomial* p = (polynomial *) calloc(3, sizeof(polynomial));

    for(int confs = 0; confs < 3 ; confs++){
      int max_coeffs = power_size[confs] > termsize ? termsize: power_size[confs];	
      p[confs].termsize = max_coeffs;
      p[confs].power = power[confs];
      p[confs].coeffs = (double *) calloc(max_coeffs, sizeof(double));
    
      for(int i=0;i<max_coeffs;i++)
	p[confs].coeffs[i] = mpq_get_d(*(prim[i].getMpqPtr_w()));
    }
    return p;
  }
...
bool rlibm_validate_and_fix_intervals(psample_data* sintervals,
  ...
  for(size_t i = 0; i < ssize; i++){
    double y = rlibm_poly_evaluation(sintervals[i].x, &poly[sintervals[i].rep]);
...
size_t rlibm_compute_violated_indices(size_t* violated_indices, pinterval_data* intervals, size_t nentries, polynomial* poly){
  ...
  for(size_t i = 0; i < nentries; i++){
    double y = rlibm_poly_evaluation(intervals[i].x, &poly[intervals[i].rep]);
...
int main(int argc, char** argv){
  ...
  for(int i = 0; i < N_RLIBM_PIECES; i++){
	...
	if(n_violated_indices <= VIOLATE_THRESHOLD){
	  ...
	  printf("bfloat  polynomial is:\n");
	  rlibm_print_polyinfo(&p[0]);

	  printf("tfloat  polynomial is:\n");
	  rlibm_print_polyinfo(&p[1]);

	  printf("float  polynomial is:\n");
	  rlibm_print_polyinfo(&p[2]);
